...
var edit_mode = '1', // 再編輯模式 ( '1'=開; '0'=不開 )
		ajax_php_url = js_url.replace('/all.js','/comments-ajax.php'),
...
$(function(){
	$("p").mouseover(function(){
		if(!$(this).is(":animated")){
			$(this).siblings().eq(0).find("img").animate({top:-6},210)
			.animate({top:0},150).animate({top:-3},150).animate({top:0},100)
...
//比例缩放滑动
        $('.boxgrid.thecombo').hover(function(){
                $(".cover", this).stop().animate({top:'100px', left:'100px'},{queue:false,duration:300});
        }, function() {
...
$('#close-sidebar a').toggle(function(){
	$(this).text("显示侧边栏");
	$('#sidebar').hide().prev().animate({width: "975px"}, 1000);
	},function(){
	$(this).text("关闭侧边栏");
	$('#sidebar').delay(800).show(0).prev().animate({width: "710px"}, 800);
});
...
function scrollBg(){
	...
	jQuery('#header').css("background-position",current+"px 0");
}
